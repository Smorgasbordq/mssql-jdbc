...
//
package com.microsoft.sqlserver.jdbc.jtds;
...
public class NtlmAuth {
	...
	private static boolean hasHmac5Support = true;
	...
	// -------------------------------------------------------------------------
	public static byte[] answerNtlmv2Challenge(String domain, String user, String password, byte[] nonce,
			byte[] targetInfo, byte[] clientNonce) {
		return answerNtlmv2Challenge(domain, user, password, nonce, targetInfo, clientNonce,
				System.currentTimeMillis());
	}

	public static byte[] answerNtlmv2Challenge(String domain, String user, String password, byte[] nonce,
			byte[] targetInfo, byte[] clientNonce, byte[] timestamp) {
	...
	public static byte[] answerNtlmv2Challenge(String domain, String user, String password, byte[] nonce,
			byte[] targetInfo, byte[] clientNonce, long now) {
		return answerNtlmv2Challenge(domain, user, password, nonce, targetInfo, clientNonce, createTimestamp(now));
	}

	public static byte[] answerLmv2Challenge(String domain, String user, String password, byte[] nonce,
			byte[] clientNonce) {
		byte[] hash = ntv2Hash(domain, user, password);
	...
	private static byte[] ntv2Hash(String domain, String user, String password) {
		...
		byte[] identityBytes;
		identityBytes = identity.getBytes(StandardCharsets.UTF_16LE);
	...
	/**
	 * Built-In methodology to calculate the HMAC-MD5 hash of the given data using
	...
	/**
	 * Creates a timestamp in the format used in NTLMv2 responses. Public so it
	...
	private static byte[] createBlob(byte[] targetInformation, byte[] clientChallenge, byte[] timestamp) {
		...
		byte[] blob = new byte[blobSignature.length + reserved.length + timestamp.length + clientChallenge.length
	...
	/**
	 * Creates the md4 hash of the Unicode password. This is used as the DES key
	 * when encrypting the nonce for NTLM challenge-response
	 */
	...
	/**
	 * Used by answerNtlmChallenge. We need the password converted to caps, narrowed
	 * and padded/truncated to 14 chars...
	 */
	private static byte[] convertPassword(String password) {
		try {
			byte[] pwd = password.toUpperCase(Locale.ENGLISH).getBytes(StandardCharsets.UTF_8);

			byte[] rtn = new byte[14];
			Arrays.fill(rtn, (byte) 0);
			System.arraycopy(pwd, 0, // src
					rtn, 0, // dst
