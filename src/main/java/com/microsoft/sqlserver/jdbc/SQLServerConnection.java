 ...
 * created from it. SQLServerConnection may participate in XA distributed
 * transactions managed via an XAResource adapter.
 * <p>
 * SQLServerConnection instantiates a new TDSChannel object for use by itself
 * and all statement objects that are created under this connection.
 * SQLServerConnection is thread safe.
 * <p>
 * SQLServerConnection manages a pool of prepared statement handles. Prepared
 * statements are prepared once and typically executed many times with different
 * data values for their parameters. Prepared statements are also maintained
 * across logical (pooled) connection closes.
 * <p>
 * SQLServerConnection is not thread safe, however multiple statements created
 * from a single connection can be processing simultaneously in concurrent
 * threads.
 * <p>
 * This class's public functions need to be kept identical to the
...
public class SQLServerConnection implements ISQLServerConnection, java.io.Serializable {
	...
	// Thresholds related to when prepared statement handles are cleaned-up. 1 ==
	// immediately.
	/**
	 * The default for the prepared statement clean-up action threshold (i.e. when
	 * sp_unprepare is called).
	 */
	...
	/**
	 * The default for if prepared statements should execute sp_executesql before
	 * following the prepare, unprepare pattern.
	 */
	...
	class PreparedStatementHandle {
		...
		/**
		 * Specify that this statement has been explicitly discarded from being used by
		 * the cache.
		 */
		 ...
		 * 
		 * @return false: Handle could not be discarded, it is in use. true: Handle was
		 *         successfully put on path for discarding.
		 */
		...
		/**
		 * Returns whether this statement has been discarded and can no longer be
		 * re-used.
		 */
		 ...
		 * 
		 * @return false: Reference could not be added, statement has been discarded or
		 *         does not have a handle associated with it. true: Reference was
		 *         successfully added.
		 */
	...
	static {
		parsedSQLCache = new Builder<Sha1HashKey, ParsedSQLCacheItem>().maximumWeightedCapacity(PARSED_SQL_CACHE_SIZE)
				.build();
	}

	/**
	 * Get prepared statement cache entry if exists, if not parse and create a new
	 * one
	 */
	...
	/**
	 * Checks whether statement pooling is enabled or disabled. The default is set
	 * to true;
	 */
	 ...
	 * 
	 * @param sql SQL text to parse for number of parameters to intialize.
	 */
	...
	/**
	 * Struct encapsulating the data to be sent to the server as part of Federated
	...
	private enum State {
		...
		Opened, // indicates that the prelogin, login have completed, the database session
				// established and the connection is ready for use.
	...
	/*
	 * Connection state variables. NB If new state is added then logical connections
	 * derived from a physical connection must inherit the same state. If state
	 * variables are added they must be added also in connection cloning method
	 * clone()
	 */
	...
	// Translates the serverName from Unicode to ASCII Compatible Encoding (ACE), as
	// defined by the ToASCII operation of RFC 3490
	...
	/**
	 * boolean value for deciding if the driver should use bulk copy API for batch
	 * inserts
	 */
	...
	/**
	 * Registers key store providers in the
	 * globalCustomColumnEncryptionKeyStoreProviders.
	 * 
	 * @param clientKeyStoreProviders a map containing the store providers
	 *                                information.
	...
	/**
	 * Updates the columnEncryptionTrustedMasterKeyPaths with the new Server and
	 * trustedKeyPaths.
	 * 
	...
	/**
	 * Removes the trusted Master key Path from the
	...
	private int NTLM = SQLServerDriverIntProperty.NTLM.getDefaultValue();
	...
	// This is the current connect place holder this should point one of the primary
	// or failover place holder
	...
	// The flag's value is reset to false when a new transaction starts or when the
	// autoCommit mode changes.
	...
	final void setMaxFieldSize(int limit) throws SQLServerException {
		...
		if (maxFieldSize != limit) {
			...
			// If no limit on field size, set text size to max (2147483647), NOT default (0
			// --> 4K)
	...
	/**
	 * Check if the connection is closed Create a new connection if it's a fedauth
	 * connection and the access token is going to expire.
	 * 
		...
		for (int retryAttempt = 0;;) {
			try {
				...
				if (SQLServerException.DRIVER_ERROR_INTERMITTENT_TLS_FAILED != e.getDriverErrorCode()) {
					...
					if (0 == retryAttempt) {
						// We do not need to check for exceptions here, as the connection properties are
						// already
	...
	private void registerKeyStoreProviderOnConnection(String keyStoreAuth, String keyStoreSecret,
		...
		if (null == keyStoreAuth) {
			...
			default:
				// valueOfString would throw an exception if the keyStoreAuthentication is not
				// valid.
				break;
			}
		}
	...
	Connection connectInternal(Properties propsIn, SQLServerPooledConnection pooledConnection)
		...
		try {
			...
			// hostNameInCertificate property can change when redirection is involved, so
			// maintain this value
			...
			if (null != originalHostNameInCertificate && !originalHostNameInCertificate.isEmpty()) {
				...
				// reset hostNameInCertificate to the original value every time we connect (or
				// re-connect).
			...
			// get the server name from the properties if it has instance name in it,
			// getProperty the instance name
			// if there is a port number specified do not get the port number from the
			// instance name
			...
			// Set requestedEncryptionLevel according to the value of the encrypt connection
			// property
			...
			// Ignore authenticationScheme setting if integrated authentication not
			// specified
			...
			sPropKey = SQLServerDriverIntProperty.NTLM.toString();
			sPropValue = activeConnectionProperties.getProperty(sPropKey);
			if (sPropValue != null) {
				NTLM = Integer.parseInt(sPropValue);
			}

			// Force NTLM on if setting provided with integratedSecurity
			if (integratedSecurity && NTLM > 0) {
				intAuthScheme = AuthenticationScheme.NTLM;
			}
			...
			// transparentNetworkIPResolution is ignored if multiSubnetFailover or
			// DBMirroring is true and user does not set TNIR explicitly
			...
			// that the final negotiated TDS packet size is no larger than the SSL record
			// size.
			if (TDS.ENCRYPT_ON == negotiatedEncryptionLevel || TDS.ENCRYPT_REQ == negotiatedEncryptionLevel) {
				// IBM (Websphere) security provider uses 8K SSL record size. All others use
				// 16K.
	...
	// This function is used by non failover and failover cases. Even when we make a
	// standard connection the server can provide us with its
	// FO partner.
	...
	private void login(String primary, String primaryInstanceName, int primaryPortNumber, String mirror,
		...
		// time out would be used as time slice.
		...
		while (true) {
			...
			try {
				...
				if (isRoutedInCurrentAttempt) {
					...
					// is used as a one full interval
				...
				// sleep interval after every attempt.
			...
			// timeOutInterval is full timeout would also be handled correctly.
		...
		if (null != failoverPartnerServerProvided) {
			...
			// from the map just make sure that we change the
			if (null != foActual) {
	...
	static int TimerRemaining(long timerExpire) {
		...
		// we want atleast one attempt to happen with a positive timeout passed by the
		// user.
		if (result <= 0)
			result = 1;
		return (int) result;
	 ...
	 * connect and the server name to connect and the timeout This function achieves
	 * one connection attempt Create a prepared statement for internal use by the
	 * driver.
	 * 
	 * @param serverInfo
	 * @param timeOutSliceInMillis               -timeout value in milli seconds for
	 *                                           one try
	 * @param timeOutFullInSeconds               - whole timeout value specified by
	 *                                           the user in seconds
	 * @param useParallel                        - It is used to indicate whether a
	 *                                           parallel algorithm should be tried
	 *                                           or not for resolving a hostName.
	 *                                           Note that useParallel is set to
	 *                                           false for a routed connection even
	 *                                           if multiSubnetFailover is set to
	 *                                           true.
	 * @param useTnir
	...
	private void connectHelper(ServerPortPlaceHolder serverInfo, int timeOutsliceInMillis, int timeOutFullInSeconds,
		...
		// as the InetAddress.getLocalHost() takes more than usual time in certain OS
		// and JVM combination, it avoids connection loss
	...
	/**
	 * Terminates the connection and throws an exception detailing the reason for
	 * termination.
	 *
	 * This method is similar to SQLServerException.makeFromDriverError, except that
	 * it always terminates the connection, and does so with the appropriate state
	 * code.
	 */
	...
	public void close() throws SQLServerException {
		...
		// Clean-up queue etc. related to batching of prepared statement discard actions
		// (sp_unprepare).
	...
	// This function is used by the proxy for notifying the pool manager that this
	...
	final void poolCloseEventNotify() throws SQLServerException {
		if (state.equals(State.Opened) && null != pooledConnectionParent) {
			...
			// XA = true => the transaction manager is the only one who can invoke
			// transactional APIs

			// Non XA and autocommit off =>
			// If there is a pending BEGIN TRAN from the last commit or rollback, dont
			// propagate it to
			// the next allocated connection.
			// Also if the app closes a connection handle before committing or rolling back
			// the uncompleted
	...
	public void setTypeMap(java.util.Map<String, Class<?>> map) throws SQLException {
		...
		if (map != null && (map instanceof java.util.HashMap)) {
			// we return an empty Hash map if the user gives this back make sure we accept
			// it.
			if (map.isEmpty()) {
	...
	{
		// This includes the length of the terminator byte. If there are other extension
		// features, re-adjust accordingly.
	...
	int writeDataClassificationFeatureRequest(boolean write /* if false just calculates the length */,
		...
		int len = 6; // 1byte = featureID, 4bytes = featureData length, 1 bytes = Version
		if (write) {
			// Write Feature ID, length of the version# field and Sensitivity Classification
			// Version#
	...
	int writeUTF8SupportFeatureRequest(boolean write, TDSWriter tdsWriter /* if false just calculates the length */)
			throws SQLServerException {
	...
	private void logon(LogonCommand command) throws SQLServerException {
		...
		if (integratedSecurity && AuthenticationScheme.NTLM == intAuthScheme) {
			authentication = new NtlmAuthentication(this);
		}

		// If the workflow being used is Active Directory Password or Active Directory
		// Integrated and server's prelogin response
		// for FEDAUTHREQUIRED option indicates Federated Authentication is required, we
		// have to insert FedAuth Feature Extension
		// in Login7, indicating the intent to use Active Directory Authentication
		...
		if (null != accessTokenInByte) {
			...
			// No need any further info from the server for token based authentication. So
			// set _federatedAuthenticationRequested to true
	...
	final void processEnvChange(TDSReader tdsReader) throws SQLServerException {
		...
		case ENVCHANGE_XACT_ROLLBACK:
			...
			if (inXATransaction) {
				...
				// For a DTC transaction, a ENV_ROLLBACKTRAN token won't cleanup the xactID
				// previously cached on the connection
				// because user is required to explicitly un-enlist/defect a connection from a
				// DTC.
		...
		case ENVCHANGE_ROUTING:
			...
			// Check if the hostNameInCertificate needs to be updated to handle the rerouted
			// subdomain in Azure
			...
			if (null != currentHostName && currentHostName.startsWith("*") && (null != routingServerName) /*
				...
				/*
				 * Check if routingServerName and hostNameInCertificate are from same domain by
				 * verifying each character in currentHostName from last until it reaches the
				 * character before the wildcard symbol (i.e. currentHostNameCharArray[1])
				 */
		...
		// After extracting whatever value information we need, skip over whatever is
		// left
	...
	final void processFedAuthInfo(TDSReader tdsReader, TDSTokenHandler tdsTokenHandler) throws SQLServerException {
		...
		if (tokenLen < 4) {
			// the token must at least contain a DWORD(length is 4 bytes) indicating the
			// number of info IDs
			if (connectionlogger.isLoggable(Level.SEVERE)) {
		...
		if (tokenLen > 0) {
			...
			for (int i = 0; i < optionsCount; i++) {
				...
				// if dataOffset points to a region within FedAuthInfoOpt or after the end of
				// the token, throw
				...
				// try to read data and throw if the arguments are bad, meaning the server sent
				// us a bad token
				String data = null;
				try {
	...
	/**
	 * Generates (if appropriate) and sends a Federated Authentication Access token
	 * to the server, using the Federated Authentication Info.
	 */
	...
	private SqlFedAuthToken getFedAuthToken(SqlFedAuthInfo fedAuthInfo) throws SQLServerException {
		...
		while (true) {
			if (authenticationString.trim().equalsIgnoreCase(SqlAuthentication.ActiveDirectoryPassword.toString())) {
				...
				// If operating system is windows and sqljdbc_auth is loaded then choose the DLL
				// authentication.
				if (System.getProperty("os.name").toLowerCase(Locale.ENGLISH).startsWith("windows")
					...
					try {
						...
						// the sqljdbc_auth.dll return -1 for errorCategory, if unable to load the
						// adalsql.dll
				...
				// else choose ADAL4J for integrated authentication. This option is supported
				// for both windows and unix, so we don't need to check the
				// OS version here.
				else {
	...
	private void onFeatureExtAck(byte featureId, byte[] data) throws SQLServerException {
		...
		case TDS.TDS_FEATURE_EXT_FEDAUTH: {
			...
			case TDS.TDS_FEDAUTH_LIBRARY_ADAL:
			case TDS.TDS_FEDAUTH_LIBRARY_SECURITYTOKEN:
				// The server shouldn't have sent any additional data with the ack (like a
				// nonce)
				if (0 != data.length) {
					if (connectionlogger.isLoggable(Level.SEVERE)) {
						connectionlogger.severe(toString()
	 ...
	 * 
	 * @param secsTimeout (optional) if non-zero, seconds to wait for logon to be
	 *                    sent.
	 * @throws SQLServerException
	 */
	private void sendLogon(LogonCommand logonCommand, SSPIAuthentication authentication,
		...
		// Cannot use SSPI when server has responded 0x01 for FedAuthRequired PreLogin
		// Option.
		...
		String serverName;
		// currentConnectPlaceHolder should not be null here. Still doing the check for
		// extra security.
		...
		// only add lengths of password and username if not using SSPI or requesting
		...
		// if we are using SSPI or fed auth ADAL, do not send over username/password,
		// since we will use SSPI instead
		if (!integratedSecurity && !(federatedAuthenticationInfoRequested || federatedAuthenticationRequested)) {
	...
	/**
	 * Checks that the holdability argument is one of the values allowed by the JDBC
	 * spec and by this driver.
	 */
	 ...
	 * Statement.getResultSetHoldability to check the holdability of ResultSets that
	 * would be created, and/or ResultSet.getHoldability to check the holdability of
	...
	final private Savepoint setNamedSavepoint(String sName) throws SQLServerException {
		...
		// statements. Also note that the way we create this transaction is rather
		// weird.
		// This is because the server creates a nested transaction (@@TRANCOUNT = 2)
		// rather
		// than just the outer transaction (@@TRANCOUNT = 1). Should this limitation
		// ever
	...
	public void setClientInfo(Properties properties) throws SQLClientInfoException {
		...
		// This function is only marked as throwing only SQLClientInfoException so the
	...
	String replaceParameterMarkers(String sqlSrc, Parameter[] params, boolean isReturnValueSyntax)
			throws SQLServerException {
	...
	// Notify any interested parties (e.g. pooling managers) of a ConnectionEvent
	// activity
	// on the connection. Calling notifyPooledConnection with null event will place
	// this
	// connection back in the pool. Calling notifyPooledConnection with a non-null
	// event is
	// used to notify the pooling manager that the connection is bad and should be
	// removed
	// from the pool.
	...
	String getInstancePort(String server, String instanceName) throws SQLServerException {
		...
		try {
			...
			// Second, we need to get the IP address of the server to which we'll send the
			// UDP request.
			// This may require a DNS lookup, which may fail due to transient conditions, so
			// retry after logging the first time.

			// send UDP packet
			assert null != datagramSocket;
			try {
				if (multiSubnetFailover) {
					// If instance name is specified along with multiSubnetFailover, we get all IPs
					// resolved by server name
					...
					// If instance name is not specified along with multiSubnetFailover, we resolve
					// only the first IP for server name
	...
	// ColumnEncryptionKeyCache sets time-to-live for column encryption key entries
	// in
	// the column encryption key cache for the Always Encrypted feature. The default
	// value is 2 hours.
	...
	/**
	 * Sets time-to-live for column encryption key entries in the column encryption
	 * key cache for the Always Encrypted feature. The default value is 2 hours.
	 * This variable holds the value in seconds.
	 * 
	 ...
	 * 
	 * @param statementHandle The prepared statement handle that should be scheduled
	 *                        for unprepare.
	 */
	...
	/**
	 * Remove references to outstanding un-prepare requests. Should be run when
	...
	/**
	 * Cleans-up discarded prepared statement handles on the server using batched
	 * un-prepare actions if the batching threshold has been reached.
	 * 
	 * @param force When force is set to true we ignore the current threshold for if
	 *              the discard actions should run and run them anyway.
	 */
	final void unprepareUnreferencedPreparedStatementHandles(boolean force) {
		// Skip out if session is unavailable to adhere to previous non-batched
		// behavior.
		if (isSessionUnAvailable())
			return;
	...
	/**
	 * Get or create prepared statement handle cache entry if statement pooling is
	 * enabled
	 */
	 ...
	 * enabled
	 */
	final PreparedStatementHandle registerCachedPreparedStatementHandle(Sha1HashKey key, int handle,
			boolean isDirectSql) {
	...
	final class PreparedStatementCacheEvictionListener
			implements EvictionListener<Sha1HashKey, PreparedStatementHandle> {
	...
	boolean isAzureDW() throws SQLServerException, SQLException {
		if (null == isAzureDW) {
			try (Statement stmt = this.createStatement();
				...
				// SERVERPROPERTY('EngineEdition') can be used to determine whether the db
				// server is SQL Azure.
				...
				// Developer.)
				// 4 = Express (This is returned for Express, Express with Advanced Services,
				// and Windows Embedded SQL.)
				// 5 = SQL Azure
				// 6 = SQL Azure DW
				// Base data type: int
	...
	/**
	 * @param st Statement to add to openStatements
	 */
	...
	/**
	 * @param st Statement to remove from openStatements
	 */
...
final class SQLServerConnectionSecurityManager {
	...
	/**
	 * Throws a <code>SecurityException</code> if the calling thread is not allowed
	 * to dynamic link the library code.
	 * 
	 * @throws SecurityException when an error occurs
	 */
